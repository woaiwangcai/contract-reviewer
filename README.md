...
* **确定性工程管线**：格式转换：将docx转换成大语言模型更易懂的MD格式、结构保真、报告渲染全部由代码确定性执行，大语言模型仅聚焦风险识别。
* **严格的数据与指令分离**：将合同全文视为纯只读输入，从机制上防御提示词注入，减少大模型虚构模型虚构条款。
* **证据链精准溯源**：审查报告强制要求每一项法律风险关联合同原文字句，杜绝空泛推论。
* **全平台模型兼容**：采用通用接口标准，无缝适配各类主流开源及商用大语言模型。
* **独立单文件报告**：解决MD文档难以阅读痛点，改为阅读难度更低的HTML报告。
...
```mermaid
flowchart LR
    A["DOCX 合同"] -->|结构化清洗与转化为| B["标准 Markdown"]
    B --> C["审查 Skill 协议"]
...
| 模块 | 文件 | 职责说明 |
| :--- | :--- | :--- |
| **文档解析** | `src/docx_to_md.py` | 严格保留标题层级、段落、列表与表格顺序等，剔除冗余干扰样式 |
| **审查协议** | `skills/contract_review.md` | 明确审查边界、证据要求、风险等级标准与结构化输出格式 |
| **模型驱动** | `src/model_client.py` | 通用接口大模型API端，可以自由配置模型厂商，负责标准化调用与安全校验 |
...
* **防覆盖机制**：若同名报告已存在，程序会自动追加序号（如 `-1`、`-2`），保障历史记录安全。
...
这里的 Skill 是一份专业约束的**审查执行标准与接口约定**，明确规定：
...
* AI 审查结果仅供合同审查辅助参考，重要法律实务规定请结合人工复核，AI亦会犯错。
